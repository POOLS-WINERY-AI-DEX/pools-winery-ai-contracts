    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(string memory baseURI) public initializer {
        __ERC721_init("Pools Winery AI NFT", "PoolsWineryAINFT");
    ...
    function getInfoForStaking(
        uint tokenId
    )
        external
        view
        returns (address tokenOwner, bool stakeFreeze, uint robiBoost)
    {
    ...
    function setRBTable(
        uint[7] calldata rbTable
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _rbTable = rbTable;
    }

    function setLevelTable(
        uint[7] calldata levelTable
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _levelTable = levelTable;
    }

    function setLevelUpPercent(
        uint8 percent
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function accrueRB(
        address user,
        uint amount
    ) external onlyRole(RB_SETTER_ROLE) {
    ...
    function exchangeRB(
        uint amount,
        address userAddress
    ) public nonReentrant onlyRole(RB_SETTER_ROLE) returns (bool) {
    ...
    function sendRBToMaxInTokenLevel(
        uint[] calldata tokenId
    ) public nonReentrant {
        require(
            tokenId.length <= MAX_ARRAY_LENGTH_PER_REQUEST,
    ...
    function _remainRBToMaxLevel(uint tokenId) internal view returns (uint) {
        return
            _rbTable[uint(_tokens[tokenId].level)] - _tokens[tokenId].robiBoost;
    }

    function _sendRBToToken(
        uint[] memory tokenId,
        uint[] memory amount
    ) internal {
        require(
